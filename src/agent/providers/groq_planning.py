"""Optional Groq Responses API adapter for provider-neutral planning."""
...
class GroqPlanningDependencyError(GroqPlanningError, ImportError):
    """Raised when the optional OpenAI SDK transport is unavailable."""


def _validate_model(model: object) -> str:
    if not isinstance(model, str) or not model.strip():
        raise ValueError("`model` must be a non-empty Groq model name.")
    return model.strip()
...
def _validate_client(client: object) -> object:
    ...
    if not callable(getattr(responses, "create", None)):
        raise TypeError(
            "Groq planning client must provide responses.create()."
        )
    return client
...
def _completed_output_text(response: object) -> str:
    ...
    try:
        ...
        raise GroqPlanningError(
            "Groq planning response text could not be read."
        ) from exc
...
class GroqPlanningModel:
    """PlanningModel adapter using one Groq Responses API request."""

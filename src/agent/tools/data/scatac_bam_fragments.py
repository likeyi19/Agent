"""Registered BAM preparation and exact durable publication receipt recovery."""
...
from . import bam_fragment_manifest as m, _bam_fragment_io as io, scatac_fragments_v2 as v2
...
RECOVERY_POLICY = 'prepare-scatac-fragments-bam-v1'


class BamFragmentsResult(TypedDict):
    status: str
    manifest_path: str
    manifest_sha256: str
    artifact_type: str
...
def _receipt(destination, arguments, expected_token=None):
    try:
        ...
        if len(raw) > 16384:
            m.fail('BAM_FRAGMENTS_RECOVERY_MISMATCH')
        ...
        if (value['artifact_type'] != 'agent.bam-fragments-execution-receipt'
                or type(value['schema_version']) is not int or value['schema_version'] != 1
                or value['policy'] != RECOVERY_POLICY or value['arguments_sha256'] != digest(arguments)
                or destination.name != 'bam-fragments-' + value['execution_identity']
...
def verify_public_result(arguments, result):
    ...
    if (path != path.resolve() or path.name != 'manifest.json' or path.parent.name != 'fragments'
        ...
        m.fail('BAM_FRAGMENTS_RESULT_MISMATCH')
    ...
    if receipt['manifest_sha256'] != result['manifest_sha256']:
        m.fail('BAM_FRAGMENTS_RECOVERY_MISMATCH')
    verified = verify_bam_fragments(path, expected_sha256=result['manifest_sha256'], runtime=verification_runtime())
    value = verified.fragments.manifest; entry = value['libraries'][0]
    record = m.load_record(entry['provenance']['producer_record']['path'], entry['provenance']['producer_record']['sha256'])
    if record['arguments'] != args or dict(result) != _summary(value, path, result['manifest_sha256']):
        m.fail('BAM_FRAGMENTS_RESULT_MISMATCH')
    io.check_snapshots(before)
    return verified


def recover_bam_fragments(arguments, execution_identity):
    args, destination, token = _publication(arguments, execution_identity)
    if not destination.is_dir() or destination.is_symlink():
        m.fail('BAM_FRAGMENTS_RECOVERY_UNAVAILABLE')
...
def execute_bam_fragments(arguments, execution_identity=None):
    from .bam_fragments import prepare_in_stage
    args, destination, token = _publication(arguments, execution_identity)
    ...
    with (output / ('.' + destination.name + '.lock')).open('a+b') as lease:
        ...
        if destination.exists() or destination.is_symlink():
            m.fail('BAM_FRAGMENTS_ARTIFACT_CONFLICT')
        stage = Path(tempfile.mkdtemp(prefix='.bam-attempt-', dir=output))
        try:
            ...
            verified = verify_bam_fragments(manifest, expected_sha256=io.file_sha256(manifest), runtime=runtime, temporary_root=stage)
            ...
            p['producer_record']['path'] = str(destination / 'fragments' / 'production.json')
            ...
            (stage / 'receipt.json').write_bytes(canonical(dict(artifact_type='agent.bam-fragments-execution-receipt',
...
def prepare_scATAC_bam_fragments(intake_manifest_path, intake_manifest_sha256,

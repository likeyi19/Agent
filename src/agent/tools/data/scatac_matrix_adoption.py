...
def arguments(value):
    ...
    if ((args['species'],args['assembly']) not in (('human','hg38'),('mouse','mm10'))
            or args['matrix_semantics'] not in e.SEMANTICS): m.fail('MATRIX_SEMANTICS_INVALID')
    return args


def _publication(args,execution_identity):
    args=arguments(args)
    token=shared.digest(dict(arguments=args,policy=RECOVERY_POLICY,execution_identity=execution_identity,matrix_profile_sha256=e.PROFILE_SHA256))
    return args,Path(args['output_dir'])/('cell-by-ccre-'+token),token


def _receipt(destination,args,token=None):
    return shared._receipt(destination,args,token,policy=RECOVERY_POLICY,profile=e.PROFILE_SHA256)


def _summary(value,path,sha):
    if value['contract_version']!=e.CONTRACT: m.fail('MATRIX_CONTRACT_INVALID')
    return dict(status='success',manifest_path=str(path),manifest_sha256=sha,artifact_type=m.ARTIFACT,
        artifact_schema_version=1,contract_version=e.CONTRACT,identity_sha256=value['identity_sha256'],
        ...
        n_cells=value['shape'][0],n_features=value['shape'][1],matrix_profile_id=e.PROFILE,matrix_profile_sha256=e.PROFILE_SHA256,
...
def verify_public_result(args,result):
    ...
    value=shared._load(path,result['manifest_sha256']); e.validate(value)
    if (value['source']['path']!=args['source_path'] or value['source']['sha256']!=args['source_sha256']
            ...
            or dict(result)!=_summary(value,path,result['manifest_sha256'])): m.fail('MATRIX_RESULT_MISMATCH')
...
def recover_matrix(arguments,execution_identity):
    ...
    result=_summary(shared._load(path,receipt['manifest_sha256']),path,receipt['manifest_sha256'])
...
def execute_matrix(arguments,execution_identity=None):
    ...
    return shared._execute_publication(args,destination,token,lambda output: scientific.build(args,output),
                                       _summary,RECOVERY_POLICY,e.PROFILE_SHA256)

...
def test_v4_complete_catalog_size_and_explicit_null_index(source_factory):
    ...
    assert size <= 34_000  # M14.7: full 23-tool audit catalog; scoped production payloads tested separately.
    import sys
    sys.path.insert(0,str(Path(__file__).parents[1]/'unit/orchestration'))
    from catalog_expansion import expand_catalog
    expanded=expand_catalog(json.loads(prompt)['catalog'])
    assert 'import_scATAC_fragments' in expanded
    assert 'Historical processing remains declared' in json.dumps(expanded)
